...
/**
 * <p>
 * This class provides functionality to execute {@link NuveiRequest}s directly to the Nuvei's REST API
 *</p>
 * @author <a href="mailto:nikolad@nuvei.com">Nikola Dichev</a>
 * @since 2/16/2017
 */
public class NuveiRequestExecutor {

    private static final Log logger = LogFactory.getLog(NuveiRequestExecutor.class);
    private static final Map<Class<? extends NuveiBaseRequest>, Class<? extends NuveiResponse>> RESPONSE_TYPE_BY_REQUEST_TYPE =
            new HashMap<Class<? extends NuveiBaseRequest>, Class<? extends NuveiResponse>>() {
    ...
    private static final Map<Class<? extends NuveiBaseRequest>, String> REQUEST_URL_BY_REQUEST_TYPE =
            new HashMap<Class<? extends NuveiBaseRequest>, String>() {
    ...
    private static NuveiRequestExecutor instance = null;
    private static HttpClient httpClient;
    private static boolean isInitialized = false;

    private NuveiRequestExecutor() {
    }

    /**
     * Obtains the single instance of {@link NuveiRequestExecutor}
     *
     * @return the single instance of {@link NuveiRequestExecutor}
     */
    public static NuveiRequestExecutor getInstance() {
        if (instance == null) {
            instance = new NuveiRequestExecutor();
        }
        return instance;
    }

    /**
     * This method initiates the {@link NuveiRequestExecutor} with a default Nuvei's {@link HttpClient} and server information.
     */
    public void init() {
        init(NuveiHttpClient.createDefault());
    }

    /**
     * This method initiates the {@link NuveiRequestExecutor} with a configured {@link HttpClient} and server information.
     *
    ...
    public void init(HttpClient httpClient) {
        ...
        if (isInitialized) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(NuveiRequestExecutor.class.getSimpleName() + " is already initialized!");
            }
            return;
        }

        NuveiRequestExecutor.httpClient = httpClient;
     ...
     */
    public NuveiResponse executeRequest(NuveiBaseRequest request) {
        try {
            return execute(request);
        } catch (NuveiException e) {
            return null;
        }
    }
     ...
     * @throws NuveiException If the request execution throws IOException it is wrapped and rethrown in {@link NuveiException}
     */
    public NuveiResponse execute(NuveiBaseRequest request) throws NuveiException {
        if (!isInitialized) {
            init();
        }
        ...
        try {
            Class<? extends NuveiBaseRequest> requestClass = request.getClass();
            ...
            NuveiResponse response = gson.fromJson(responseJSON, RESPONSE_TYPE_BY_REQUEST_TYPE.get(requestClass));
            ...
            throw new NuveiException(e.getMessage());
        }
    }
     ...
     */
    public String executeJsonRequest(String requestJSON, String serviceUrl, Class<? extends NuveiBaseRequest> requestClass) throws IOException {
     ...
     * @throws IOException if the connection is interrupted or the response is unparsable
     */
    public String executeRequest(String request, String serviceUrl, Header[] headers, Class<? extends NuveiBaseRequest> requestClass) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            Class<? extends NuveiResponse> responseClass = RESPONSE_TYPE_BY_REQUEST_TYPE.get(requestClass);
